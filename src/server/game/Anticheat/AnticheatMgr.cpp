...
{
    if ((sWorld->getConfig(CONFIG_ANTICHEAT_DETECTIONS_ENABLED) & JUMP_HACK_DETECTION) == 0)
        return;
...
{
    if ((sWorld->getConfig(CONFIG_ANTICHEAT_DETECTIONS_ENABLED) & WALK_WATER_HACK_DETECTION) == 0)
        return;
...
{
    if ((sWorld->getConfig(CONFIG_ANTICHEAT_DETECTIONS_ENABLED) & FLY_HACK_DETECTION) == 0)
        return;
...
{
    if ((sWorld->getConfig(CONFIG_ANTICHEAT_DETECTIONS_ENABLED) & TELEPORT_PLANE_HACK_DETECTION) == 0)
        return;
...
void AnticheatMgr::TeleportHackDetection(Player* player, MovementInfo movementInfo)
{
    ...
    {
        // display warning
        std::string str = "Possible \"Teleport Hacker\" (No Report).";
        BroadCastGameMasters(player, str);
    }
}
...
{
    if ((sWorld->getConfig(CONFIG_ANTICHEAT_DETECTIONS_ENABLED) & CLIMB_HACK_DETECTION) == 0)
        return;
...
{
    if ((sWorld->getConfig(CONFIG_ANTICHEAT_DETECTIONS_ENABLED) & SPEED_HACK_DETECTION) == 0)
        return;
...
std::string AnticheatMgr::TypeToString(const uint8 type) const
{
    switch (type)
    {
    ...
    case WALK_WATER_HACK_REPORT:
        return "Water Walking Hack";
        break;
    case JUMP_HACK_REPORT:
        return "Jump Hack";
        break;
    case TELEPORT_PLANE_HACK_REPORT:
        return "TelePort to Plane Hack";
        break;
...
void AnticheatMgr::BuildReport(Player* player,uint8 reportType)
{
    ...
    if (MustCheckTempReports(reportType))
    {
        ...
        if (getMSTimeDiff(m_Players[key].GetTempReportsTimer(reportType), actualTime) < REPORT_TIME_THRESHOLD)
        {
            m_Players[key].SetTempReports(m_Players[key].GetTempReports(reportType) + 1, reportType);

            if (m_Players[key].GetTempReports(reportType) < REPORT_THRESHOLD)
                return;
        }
        else
        {
    ...
    if (sWorld->getConfig(CONFIG_ANTICHEAT_MAX_REPORTS_FOR_DAILY_REPORT) < m_Players[key].GetTotalReports())
    {
    ...
    if (m_Players[key].GetTotalReports() > sWorld->getConfig(CONFIG_ANTICHEAT_REPORTS_INGAME_NOTIFICATION))
    {
        ...
        {
            // display warnings to GM
            std::string str = "Possible \"" + TypeToString(reportType)+"\".";
            BroadCastGameMasters(player, str);

            if (sWorld->getConfig(CONFIG_ANTICHEAT_BAN_ENABLE))
            {
                std::string accountName = "";
                std::string reason = "AntiCheat Violation (" + TypeToString(reportType) + ").";
                std::string time = "1d";
                str = "Was banned by AntiCheat for hacking (" + time + ").";

                sAccountMgr->GetName(player->GetSession()->GetAccountId(), accountName);
                sWorld->BanAccount(BAN_ACCOUNT, accountName, time, reason, "AntiCheat");
                BroadCastGameMasters(player, str);
            }
            else
            {
                str = "Was kicked by AntiCheat for hacking.";
...
void AnticheatMgr::BroadCastGameMasters(Player* player, std::string text) const
{
    if (text.empty())
        return;
    ...
    tempStr = "|cFFFFFC00[AntiCheat]: |cffffffff|Hplayer:" + std::string(playerName.c_str()) + "|h[" + playerName.c_str() + "]|h|r" + "|cFF00FFFF " + text;
    sWorld->SendGMText(LANG_GM_BROADCAST, tempStr.c_str());
}
